...
def build_market_summary(all_trades: List[pd.DataFrame]) -> pd.DataFrame:
  ...
  for tf in TIMEFRAMES:
    tf_trades = [
      t[t["timeframe"] == tf]
      for t in all_trades
      if not t.empty and "timeframe" in t.columns
    ]
    if not tf_trades:
      continue
    ...
    total = len(combined)
    wins = (
      len(combined[combined["pnl_abs"].astype(float) > 0])
      if "pnl_abs" in combined.columns
      else 0
    )
    losses = (
      len(combined[combined["pnl_abs"].astype(float) < 0])
    ...
    pnl_pcts = (
      combined["pnl_pct"].astype(float) if "pnl_pct" in combined.columns else []
    )
...
def build_market_all_summary(
  events_df: pd.DataFrame, trades_df: pd.DataFrame
) -> pd.DataFrame:
  ...
  for symbol in symbols:
    for tf in TIMEFRAMES:
      tf_ev = events_df[
        (events_df["symbol"] == symbol) & (events_df["timeframe"] == tf)
      ]
      ...
      if not trades_combined.empty:
        tf_trades = trades_combined[
          (trades_combined["symbol"] == symbol) & (trades_combined["timeframe"] == tf)
        ]
...
def build_symbol_digest(all_events: Dict[str, pd.DataFrame]) -> pd.DataFrame:
  ...
  for symbol in sorted(symbols):
    ...
    for tf in TIMEFRAMES:
      ...
      if ev is not None and not ev.empty and "event_date" in ev.columns:
        valid = ev["event_date"].dropna().astype(str).str[:10]
...
def build_market_digest(market_events: List[pd.DataFrame]) -> pd.DataFrame:
  ...
  for symbol in symbols:
    ...
    valid_dates = (
      sym_ev["event_date"].dropna().astype(str).str[:10]
      if "event_date" in sym_ev.columns
      else pd.Series([], dtype=str)
    )
    valid_dates = valid_dates[valid_dates.str.match(r"\d{4}-\d{2}-\d{2}")]
    ref_date = (
      pd.to_datetime(valid_dates.max()).strftime("%Y/%m/%d")
...
def main():
  ...
  for symbol in symbols:
      ...
      try:
        ...
        if not events_df.empty:
          ...
          if "event_date" in events_df.columns:
            events_df["event_date"] = events_df["event_date"].apply(
    ...
    if symbol_events:
      ...
      for tf in TIMEFRAMES:
        tf_ev = (
          symbol_all[symbol_all["timeframe"] == tf]
      ...
      if not symbol_digest.empty:
        save_df(
          symbol_digest,
          out_dir / f"{symbol}_signal_digest_last_per_symbol_v5_macdtd.csv",
        )
  ...
  print(
    f"DEBUG all_events len={len(all_events)}, market_signal_events len={len(market_signal_events)}"
  )
  if market_signal_events:
    ...
    if not market_digest.empty:
      save_df(
        market_digest, out_dir / "market_signal_digest_last_per_symbol_v5_macdtd.csv"
      )
      print(f"Market digest: {len(market_digest)} rows")
  # Build per-symbol×timeframe market_all_summary similar to v7
  try:
    print(
      f"DEBUG building market_all_summary from events ({len(market_signal_events)}) and trades ({len(all_trades)})"
    )
    events_concat = (
      pd.concat(market_signal_events, ignore_index=True)
      if market_signal_events
      else pd.DataFrame()
    )
    trades_concat = (
