...
app = FastAPI(title="Telegram Notify API", version="0.7.0")
...
def load_digest_rows(
  ...
  if not digest_path.exists():
    raise HTTPException(status_code=404, detail=f"digest file not found: {digest_path}")
  try:
...
def get_me():
  ...
  return {"ok": True, "telegram": data.get("result", {})}
...
def send_digest(req: TelegramDigestRequest):
  ...
  rows = load_digest_rows(
    ...
    ignore_event_type_whitelist=req.ignore_event_type_whitelist,
  ...
  if not messages:
    return {
      ...
      "include_empty_summary": req.include_empty_summary,
      "ignore_event_type_whitelist": req.ignore_event_type_whitelist,
      "reset_sent_state_before_send": req.reset_sent_state_before_send,
    }
